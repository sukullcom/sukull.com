...
import { BookOpen, Code, Grid, Layers, Search as SearchIcon, Tag, X } from "lucide-react";
import SnippetsPageSkeleton from "./snippets-skeleton";
...
type Snippet = {
  ...
  description: string;
  language: string;
  // code: string; // We do NOT display code here
  createdAt: string;
};
...
export default function SnippetsPage() {
  ...
  useEffect(() => {
    ...
    async function loadSnippets() {
      ...
      try {
        ...
        // We handle text searching on the server side
        // so we only pass 'search' if we have one.
        if (searchQuery.trim() !== "") {
  ...
  // 2) Once we have all snippets, compute popular languages, apply language filter
  const languages = snippets ? Array.from(new Set(snippets.map((s) => s.language))) : [];
  const popularLanguages = languages.slice(0, 5); // top 5 for your filter bar

  const filteredSnippets = (snippets || []).filter((snippet) => {
    // If a language is chosen, snippet must match it
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0a0f]">
        {/* You can also have your own nav or header here if you like */}
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      {/* If you have a NavigationHeader (like the example), you can import it here */}
      {/* <NavigationHeader /> */}
            ...
            {popularLanguages.map((lang) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <img src={`/language_logos/${lang}.png`} alt={lang} className="w-4 h-4 object-contain" />
