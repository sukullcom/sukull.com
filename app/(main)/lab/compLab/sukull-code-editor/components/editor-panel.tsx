...
import { useSearchParams } from "next/navigation"; // <-- For reading snippetId from URL
...
function EditorPanel() {
  ...
  // 1) When the editor is ready, load code from snippet or localStorage
  useEffect(() => {
    if (!editor) return; // Only proceed if editor is mounted

    // If there's a snippetId, fetch snippet from server
    if (snippetId) {
      (async () => {
        try {
          ...
          if (snippetData?.code) {
            // Put snippet's code in the editor
            editor.setValue(snippetData.code);
          } else {
            // If snippet was found but no code property
            console.warn("Snippet has no code property.");
          }
        } catch (err) {
      ...
      // If no snippetId, fallback to localStorage or default code
  ...
  // 2) Load saved fontSize on mount
  useEffect(() => {
    const savedFontSize = localStorage.getItem("editor-font-size");
    if (savedFontSize) setFontSize(parseInt(savedFontSize));
  }, [setFontSize]);

  // 3) Helper to refresh the code to default (and clear localStorage)
  const handleRefresh = () => {
  ...
  // 4) On every editor change, save to localStorage (only if no snippet is loaded? your call)
