...
import { CheckCircle, XCircle, Mail, Phone, User, BookOpen, Clock } from "lucide-react";
        ...
        <Card>
          <CardContent className="py-8">
            <p className="text-center text-gray-500">
              {!Array.isArray(applications) ? "Veri yüklenirken bir hata oluştu." : "Henüz başvuru bulunmuyor."}
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent>
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{application.studentPhoneNumber}</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <BookOpen className="h-4 w-4 text-gray-500" />
                      <span className="text-sm">{application.subject}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-gray-500" />
                      <span className="text-sm">{application.lessonDuration} dakika</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    {getLevelBadge(application.studentLevel)}
                  </div>
                ...
                <div className="mb-4">
                  <span className="text-sm text-gray-600">
                    Müsait Saatler: <strong>{application.availableHours}</strong>
                  </span>
                </div>
