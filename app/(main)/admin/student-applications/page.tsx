...
/**
 * See `teacher-applications/page.tsx` for the overall design rationale
 * — this file follows the same server-paginated pattern, differing
 * only in row shape and action verbs.
 */
            ...
            <Card key={app.id} className="overflow-hidden">
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2 text-sm mb-4">
                  ...
                  {app.studentLevel && (
                    <InfoRow icon={GraduationCap} label="Seviye" value={app.studentLevel} />
                  )}
                  {app.lessonDuration && (
                    <InfoRow
                      icon={Timer}
                      label="Ders Süresi"
                      value={`${app.lessonDuration} dk`}
                    />
                  )}
                  {app.availableHours && (
