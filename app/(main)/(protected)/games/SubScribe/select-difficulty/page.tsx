...
function SelectDifficultyContent() {
  ...
  return (
    <div className="w-full max-w-lg mx-auto flex flex-col gap-6 py-8 px-4">
      ...
      <h1 className="text-2xl font-bold text-foreground">Zorluk Seç</h1>

      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-2">Nasıl Çalışır?</h2>
        <div className="max-h-64 overflow-y-auto grid grid-cols-1 gap-2 scrollbar-thin scrollbar-thumb-suk-brand/40 scrollbar-track-muted pr-2">
          <p>
        ...
        {difficulties.map((d) => {
          ...
          return (
            <button
              ...
              className="w-full p-4 rounded-xl border-2 text-left transition-all border-border hover:border-suk-payment hover:bg-suk-payment-soft/40 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isLoading}
            >
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-medium text-suk-payment-soft-fg bg-suk-payment-soft rounded-full px-2.5 py-1">
                  x{mult} puan
                </span>
              </div>
            </button>
