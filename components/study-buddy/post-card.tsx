...
const PURPOSE_COLORS: Record<string, string> = {
  "YKS Sınavı": "bg-suk-danger-soft text-suk-danger",
...
export function PostCard({ post, currentUser, isOwnPost, onChatRequest, onEdit, onDelete }: PostCardProps) {
  const badgeColor = PURPOSE_COLORS[post.purpose] || "bg-muted text-muted-foreground";

  return (
    <div className="bg-card border border-border rounded-xl p-4 sm:p-5 hover:border-border/80 transition-colors">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {post.userSchoolName && (
            <p className="text-xs text-muted-foreground mb-2 truncate">{post.userSchoolName}</p>
          )}
