...
- **Supabase Auth**: Dashboard → **Authentication → Rate limits** ve (varsa) **özel SMTP** saatlik gönderim tavanı; büyük duyuru öncesi [üretim checklist](https://supabase.com/docs/guides/platform/going-into-prod#availability) ile hizala. Kayıt **e-posta doğrulama gerektirmez** (`signUpWithEmail` → admin `createUser` + anında oturum). `NEXT_PUBLIC_APP_URL` üretimde OAuth / şifre sıfırlama callback’i için zorunlu kalır.
- **Uygulama**: E-posta kaydı `signUpWithEmail` server action ile **IP başına** `RATE_LIMITS.signupIp` (varsayılan **500/saat**, `lib/rate-limit-db.ts`) uygulanır. Üretimde şifre sıfırlama e-postası için **özel SMTP (ör. Resend)** + Supabase Dashboard rate limit ayarları ile hizalayın.
